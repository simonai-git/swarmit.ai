...
export default function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const updateGithubRepo = async (repoValue: string) => {
    try {
      const res = await fetch(`/api/projects/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ github_repo: repoValue || null }),
      });
      if (res.ok) {
        ...
        const error = await res.json();
        alert(error.error || 'Failed to update GitHub repo');
      }
    } catch (error) {
                  ...
                  {project.github_repo ? (
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm placeholder-white/30 focus:outline-none focus:border-blue-500/50 w-48"
                        autoFocus
                      />
                      <button
                        onClick={() => updateGithubRepo(repoInputValue)}
                        disabled={!repoInputValue.includes('/')}
                        className="px-3 py-1.5 bg-blue-500/20 text-blue-400 rounded-lg text-xs hover:bg-blue-500/30 disabled:opacity-50"
                      >
                        Save
                      </button>
                      <button
                        onClick={() => { setShowRepoInput(false); setRepoInputValue(''); }}
                        className="px-3 py-1.5 text-white/50 text-xs hover:text-white/70"
                      >
                        Cancel
                      </button>
                    </div>
